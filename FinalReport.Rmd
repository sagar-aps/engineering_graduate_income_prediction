...
We will need to swswitch back to string datatype for cleaning operations but this is easily done with the "as" family of functions inside "mutate".
...
Cleaning these fields necessitates some background knowledge of the education system in India. The rationale is given in the following paragraph.
...
First, we will identify all typing mistakes, alternate names and research school names to fin the correct board by row and add them to a variable storing alternate names. Next,we can replace the names and group to see the result. We will replace all the 0s with NA.
...
After adjusting the groups as necessary, we can mutate the original dataset keeping the originals if we need them later.
